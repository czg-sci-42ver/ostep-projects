...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    for(i = POWER_SMALLEST; i <= power_largest; i++) {
        ...
        if (p->slabs != 0) {
            ...
            APPEND_NUM_STAT(i, "mul_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].mul_hits);
            APPEND_NUM_STAT(i, "div_hits", "%llu",
                    (unsigned long long)thread_stats.slab_stats[i].div_hits);
